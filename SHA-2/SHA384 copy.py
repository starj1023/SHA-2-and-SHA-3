...
def apply_adders_and_handle_carry0(eng, w, input2, ancilla, and_ancilla):
    num = 0
    and_idx = 0
    t = 1
    stack = []
...
def apply_adders_and_handle_carry1(eng, w, stack, ancilla, num, and_ancilla):
    and_idx = 0
    t = 1
    flag = 0
    while (t):
...
def csa0_0(eng, input, input2, ancillas, outputs):
    w1 = initialize_circuit_and_qubits(input)
    w1, stack0, input2, num = apply_adders_and_handle_carry00(eng, w1, input2, ancillas[0][0]+ancillas[1][0][:62], ancillas[0][1])
    w2 = initialize_circuit_and_qubits(input2)

    w1, stack1 = apply_adders_and_handle_carry01(eng, w1, stack0, ancillas[0][0], num, ancillas[0][1])
    w2, stack2 = apply_adders_and_handle_carry(eng, w2, ancillas[1][0][62:], ancillas[1][1])
...
def SHA384_init_H(H, idx):

    H0_hex = [0xcbbb9d5dc1059ed8, 0x629a292a367cd507, 0x9159015a3070dd17, 0x152fecd8f70e5939, 0x67332667ffc00b31, 0x8eb44a8768581511, 0xdb0c2e0d64f98fa7, 0x47b5481dbefa4fa4]

    chain = make_list(H0_hex[idx], 64)
    for j in range(64):
        if chain[j] == '1':
            # X | H[i][j]
            X | H[idx][63-j]
...
def SHA384_init_K(K, idx):

    K_hex = [0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc, 0x3956c25bf348b538, 0x59f111f1b605d019, 0x923f82a4af194f9b, 0xab1c5ed5da6d8118,
    ...
    chain = make_list(K_hex[idx], 64)
    for j in range(64):
        if chain[j] == '1':
            # X | K[i][j]
            X | K[63-j]

def SHA384_preprocessing(msg, q_msg):
    global msg_bin_arr
    ...
    for i in range(lenght+1, 896):
        msg_bin += ['0']

    len_arr = make_list(lenght, 128)
    msg_bin += len_arr
...
def w_sigma(rr0, rr1, ss, input, output):
    ...
    ss_len = 64 - ss
    for i in range(64):
        CNOT | (input[(rr0+i)%64], output[i])
    for i in range(64):
        CNOT | (input[(rr1+i)%64], output[i])
    for i in range(64):
        if i < ss_len: # ss ~ 31
...
def SHA384_round(K, W, H, ancilla, add_ancilla, garbages):
    ancilla_0 = 0 # d
    ancilla_1 = 0 # h
    ...
    for i in range(80):
        if i == 0:
            w_sigma(1, 8, 7, W[idx - 15], garbages[-7])  # sigma0
        elif i >= 1 and i <= 47:
            if i <= 5 or i >= 14:
                w_sigma(1, 8, 7, W[idx - 15], ancilla_0)  # sigma0
            w_sigma(19, 61, 6, W[idx - 2], ancilla_1)  # sigma1
            if i >= 32:
                w_sigma(1, 8, 7, W[(idx+1) - 15], garbages[-7])  # sigma0
                w_sigma(19, 61, 6, W[(idx+1) - 2], garbages[-6])  # sigma1
        elif (i >= 54 and i <= 56) or (i >= 58 and i <= 65):
            w_sigma(1, 8, 7, W[dag_idx - 15], ancilla_0)  # sigma0
            w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1
            w_sigma(1, 8, 7, W[(dag_idx-1) - 15], garbages[-7])  # sigma0
            w_sigma(19, 61, 6, W[(dag_idx-1) - 2], garbages[-6])  # sigma1
        elif i >= 66 and i <= 73:
            if i == 66: ## -8:15
                init_W(garbages[-8], dag_idx-16) # W[15]
                w_sigma(1, 8, 7, W[dag_idx-15], ancilla_0)  # sigma0
                w_sigma(19, 61, 6, W[dag_idx-2], ancilla_1)  # sigma1
                w_sigma(1, 8, 7, garbages[-8], garbages[-7])  # sigma0
                w_sigma(19, 61, 6, W[(dag_idx-1)-2], garbages[-6])  # sigma1
            elif i >= 67 and i <= 69:
                w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1
                w_sigma(19, 61, 6, W[(dag_idx - 1)-2], garbages[-6])  # sigma1
            elif i == 70: ## -8:6 // ancilla_0:15
                init_W(garbages[-8], dag_idx - 17)  # W[6]
                init_W(ancilla_0, dag_idx - 8)  # W[15]
                w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1
                w_sigma(19, 61, 6, W[(dag_idx - 1)-2], garbages[-6])  # sigma1
            elif i == 71: ## -8:6
                w_sigma(1, 8, 7, garbages[-8], ancilla_0)  # sigma0
                w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1
                w_sigma(1, 8, 7, W[5], garbages[-7])  # sigma0
                w_sigma(19, 61, 6, W[(dag_idx - 1)-2], garbages[-6])  # sigma1
            elif i == 72:
                init_W(garbages[-8], dag_idx - 13)  # W[6] dag
                w_sigma(1, 8, 7, W[4], ancilla_0)  # sigma0
                w_sigma(19, 61, 6, W[dag_idx-2], ancilla_1)  # sigma1
                w_sigma(1, 8, 7, W[3], garbages[-7])  # sigma0
                w_sigma(19, 61, 6, W[(dag_idx-1)-2], garbages[-6])  # sigma1
            elif i == 73: ## -8:15
                init_W(garbages[-8], dag_idx-2)  # W[15]
                w_sigma(19, 61, 6, garbages[-8], ancilla_1)  # sigma1
                w_sigma(1, 8, 7, W[2], ancilla_0)  # sigma0
                w_sigma(1, 8, 7, W[1], garbages[-7])  # sigma0
        elif i == 74:
            final = garbages[-10:]
            idx = 0

        SHA384_init_K(K, i)
        r_sigma(28, 34, 39, h[0], ancilla[0])
        r_sigma(14, 18, 41, h[4], ancilla[1])
        ...
        elif (i >= 48 and i <= 53) or i == 57:
            output = [0, 0]
            h[7], h[3] = csa0(eng, input, input2, add_ancilla[0:2], output)

        elif (i >= 54 and i <= 56) or (i >= 58 and i <= 65): # gar ancilla dagger
            if i == 54:
        ...
        elif i >= 66 and i <= 73: # gar ancilla dagger
            if i == 66: ## -8:15
                ...
                input4 = [garbages[-8], ancilla_0, garbages[-6]]
                input3 = [W[dag_idx - 7], ancilla_1]
                output = [gg1, gg2, garbages[gar_idx_d]]
                h[7], h[3], gg1 = csa1_d(eng, input, input2, input3, add_ancilla, output)
                gg2 = csa2_d(eng, input4, tmp, garbages[gar_idx_d - 1])
                gar_idx_d -= 2
            elif i == 71 or i == 72: ## -8:6
                input3 = [W[5], ancilla_0, ancilla_1]
                input4 = [W[4], garbages[-7], garbages[-6]]
                output = [gg1, gg2, garbages[gar_idx_d]]
                h[7], h[3], gg1 = csa1_d(eng, input, input2, input3, add_ancilla, output)
                gg2 = csa2_d(eng, input4, tmp, garbages[gar_idx_d - 1])
                gar_idx_d -= 2
        ...
        if i == 0:
            ancilla_0 = SHA384_init_H(H, 3)  # dag
            ancilla_1 = SHA384_init_H(H, 7)  # dag
            w_sigma(1, 8, 7, W[idx - 15], garbages[-7])  # dag
            idx += 1
            ...
            if i <= 5 or i >= 14:
                w_sigma(1, 8, 7, W[idx - 15], ancilla_0)  # sigma0
            if i >= 32:
                w_sigma(19, 61, 6, W[(idx+1) - 2], garbages[-6])  # sigma1 dag
                w_sigma(1, 8, 7, W[(idx+1) - 15], garbages[-7])  # sigma0 dag
                idx += 1
            idx += 1

        elif (i >= 54 and i <= 56) or (i >= 58 and i <= 65):
            w_sigma(19, 61, 6, W[(dag_idx - 1) - 2], garbages[-6])  # sigma1 dag
            w_sigma(1, 8, 7, W[(dag_idx - 1) - 15], garbages[-7])  # sigma0 dag
            w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1 dag
            w_sigma(1, 8, 7, W[dag_idx - 15], ancilla_0)  # sigma0 dag
            dag_idx -= 2
            ...
            if i == 66:
                ...
                w_sigma(19, 61, 6, W[(dag_idx - 1) - 2], garbages[-6])  # sigma1
                w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1
                init_W(ancilla_0, dag_idx - 8)  # W[15] dag
                dag_idx -= 2
            elif i == 71:
                w_sigma(19, 61, 6, W[(dag_idx - 1) - 2], garbages[-6])  # sigma1
                w_sigma(1, 8, 7, W[5], garbages[-7])  # sigma0
        ...
        r_sigma(28, 34, 39, h[0], ancilla[0]) # dag
        r_sigma(14, 18, 41, h[4], ancilla[1]) # dag
        SHA384_init_K(K, i) # dag
...
def sha2(eng):
    ...
    for i in range(8):
        H.append(eng.allocate_qureg(64))

    for i in range(16):
        W.append(eng.allocate_qureg(64))

    ancilla = []
    for i in range(2):
        ancilla.append(eng.allocate_qureg(64))  # r_sigma

    add_ancilla = []
    tmp = []
    tmp.append(eng.allocate_qureg(313)) # 7 CSA
    tmp.append(eng.allocate_qureg(168)) # outDraper_62bit
    add_ancilla.append(tmp)
    tmp = []
    tmp.append(eng.allocate_qureg(188)) # 5 CSA
    tmp.append(eng.allocate_qureg(168)) # outDraper_62bit
    add_ancilla.append(tmp)
    tmp = []
    tmp.append(eng.allocate_qureg(126))  # 5 CSA
    tmp.append(eng.allocate_qureg(171))  # outDraper_63bit
    add_ancilla.append(tmp)

    garbages = []
    for i in range(48):
        garbages.append(eng.allocate_qureg(64))

    msg = "abc"

    for i in range(8):
        SHA384_init_H(H, i)
    SHA384_preprocessing(msg, W)
    SHA384_round(K, W, H, ancilla, add_ancilla, garbages)

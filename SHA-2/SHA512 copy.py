...
def SHA512_init_H(H, idx):

    H0_hex = [0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1, 0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179]
...
def SHA512_preprocessing(msg, q_msg): 
    global msg_bin_arr
...
def SHA512_round(K, W, H, ancilla, add_ancilla, garbages):
    ancilla_0 = 0 # d
    ancilla_1 = 0 # h
    ...
    for i in range(80):
        if i == 0:
            ...
            if i == 66: ## -8:15
                ...
                if i == 69: ## -8:8
                    ...
                    w_sigma(1, 8, 7, garbages[-8], ancilla_0)  
                w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1
                w_sigma(19, 61, 6, W[(dag_idx - 1)-2], garbages[-6])  # sigma1
            elif i == 70:
                init_W(garbages[-8], dag_idx - 1 - 7)  # W[15]
                w_sigma(19, 61, 6, W[dag_idx - 2], ancilla_1)  # sigma1
                w_sigma(1, 8, 7, W[7], garbages[-7])  # sigma0
                w_sigma(19, 61, 6, W[(dag_idx - 1)-2], garbages[-6])  # sigma1
            elif i == 71 or i == 72:
                if i == 71:
                ...
                w_sigma(1, 8, 7, W[dag_idx-15], ancilla_0)  # sigma0
                w_sigma(19, 61, 6, W[dag_idx-2], ancilla_1)  # sigma1
                w_sigma(1, 8, 7, W[(dag_idx-15)-1], garbages[-7])  # sigma0
                w_sigma(19, 61, 6, W[(dag_idx - 1) - 2], garbages[-6])  # sigma1
            elif i == 73: ## -8:15
        ...
        if i == 0:
            ancilla_0 = SHA512_init_H(H, 3)  # dag
            ancilla_1 = SHA512_init_H(H, 7)  # dag
            ...
            if i == 66:
                ...
                w_sigma(1, 8, 7, garbages[-8], ancilla_0)  # sigma0 dag
                dag_idx -= 2
            elif i == 71 or i == 72:
                w_sigma(19, 61, 6, W[(dag_idx - 1) - 2], garbages[-6])  # sigma1
                w_sigma(1, 8, 7, W[(dag_idx - 15) - 1], garbages[-7])  # sigma0
...
def main(eng):
    ...
    for i in range(8):
        SHA512_init_H(H, i)
    SHA512_preprocessing(msg, W)
    SHA512_round(K, W, H, ancilla, add_ancilla, garbages)

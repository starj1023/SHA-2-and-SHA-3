...
def SHA(eng):
    ...
    for round in range(24):
        ...
        if (round >= 4 and (round-4*index) % 5 == 0):
            ...
            chi_reverse(eng, reverse_target, copy_state2[2+jump], copy_state2_before_phi[3+jump])
        ...
        if (round >= 4 and (round-4*index) % 6 == 0):
            ...
            chi_reverse(eng, reverse_target, copy_state2[1+jump], copy_state2_before_phi[2+jump])
        ...
        if (round >= 4 and (round-4*index) % 7 == 0):
            if(round==23):
                ...
                chi_reverse(eng, reverse_target, copy_state2[0+jump], copy_state2_before_phi[1+jump])
        ...
        chi(eng, target, state, copy_state2[round], anc)
...
def chi(eng, copy_state, state, result_state, anc):
    for i in range(5):
        for j in range(5):
            for k in range(64):
                AND_gate(eng, copy_state[i * 5 + ((j + 1) % 5)][k], state[i * 5 + ((j + 2) % 5)][k], result_state[j + i * 5][k], anc[i][j][k])

def chi_reverse(eng, copy_state, state, result_state):
    for i in range(5):
        for j in range(5):
            for k in range(64):

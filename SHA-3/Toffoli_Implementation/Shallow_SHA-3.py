...
def SHA(eng):
    ...
    for round in range(24):
        ...
        if (round >= 4 and (round-4*index) % 5 == 0):
            ...
            chi(eng, reverse_target, copy_state2[2+jump], copy_state2_before_phi[3+jump])

            state_copy(eng, copy_state2[2+jump], reverse_target)
            state_copy(eng, copy_state2[2+jump], copy_state2_before_phi[3+jump])  # copy_state2_before_phi[round - 1] -> clean (B)
        ...
        if (round >= 4 and (round-4*index) % 6 == 0):
            ...
            chi(eng, reverse_target, copy_state2[1+jump], copy_state2_before_phi[2+jump])

            state_copy(eng, copy_state2[1+jump], reverse_target)  # Clean copy_state (save 1600 qubits)
            state_copy(eng, copy_state2[1+jump], copy_state2_before_phi[2+jump])  # copy_state2_before_phi[round - 2] -> clean (B)
        ...
        if (round >= 4 and (round-4*index) % 7 == 0):
            if(round==23):
                ...
                chi(eng, reverse_target, copy_state2[0+jump], copy_state2_before_phi[1+jump])

                state_copy(eng, copy_state2[0+jump], reverse_target)  # copy_state1 -> clean (A), note that 0b111111...
                state_copy(eng, copy_state2[0+jump], copy_state2_before_phi[1+jump])  # copy_state2_before_phi[round - 3] -> clean (A)
